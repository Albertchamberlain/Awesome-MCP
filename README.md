...
- [OrcaReplay](https://github.com/Continuum-AI-Corp/OrcaReplay) `stdio` — Record agent and provider traffic to a local trace library, then serve it over a stdio MCP server for listing, inspecting, diffing, and offline replay of previous runs. — `debugging`, `replay`, `tracing`
