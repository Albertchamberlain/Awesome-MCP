"""MCP meta-server that exposes the Awesome-MCP catalog to agents."""
...
def _entry_to_dict(entry) -> dict[str, Any]:
    return {
        ...
        "transport": list(entry.transport),
...
def search_catalog(query: str, kind: str = "", limit: int = 10) -> str:
    """Search the Awesome-MCP catalog by keywords across names, tags, and descriptions."""
    catalog = _catalog()
    resolved_kind = kind.strip() or None
    if resolved_kind and resolved_kind not in {"server", "client", "registry", "framework"}:
...
def catalog_stats() -> str:
    """Return summary statistics for the Awesome-MCP catalog."""
    catalog = _catalog()
    return json.dumps(
        {
            "meta": {
